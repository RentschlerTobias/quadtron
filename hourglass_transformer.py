...
class ShorteningLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if seq_len % factor != 0:
            ...
            padding = torch.zeros(batch_size, padding_len, d_model, device=x.device)
        ...
        if self.method == 'linear':
            # Linear projection
            x_flat = x_grouped.reshape(batch_size, seq_len // factor, factor * d_model)
            shortened = self.linear(x_flat)
        elif self.method == 'attention':
            # Attention-based pooling
            # Use the last token of each group as query
            queries = self.query_proj(x_grouped[:, :, -1, :])  # [batch, seq_len//factor, d_model]
            keys = x_grouped.reshape(batch_size * (seq_len // factor), factor, d_model)
            values = keys
            
            # Reshape queries for attention
            queries = queries.reshape(batch_size * (seq_len // factor), 1, d_model)
            
            attended, _ = self.attention(queries, keys, values)
            shortened = attended.squeeze(1).reshape(batch_size, seq_len // factor, d_model)
...
class UpsamplingLayer(nn.Module):
    ...
    def __init__(self, d_model: int, upsampling_factor: int, method: str = 'linear'):
        ...
        Args:
            ...
            method: 'repeat', 'linear', or 'attention'
        """
        super().__init__()
        self.d_model = d_model
        ...
        if method == 'linear':
            ...
            self.attention = nn.MultiheadAttention(d_model, num_heads=8, batch_first=True)
            self.pos_emb = nn.Parameter(torch.randn(1, upsampling_factor, d_model))
    ...
    def forward(self, x: torch.Tensor, target_len: Optional[int] = None) -> torch.Tensor:
        ...
        if self.method == 'repeat':
            # Simple repetition
            upsampled = x.unsqueeze(2).expand(-1, -1, factor, -1)
            upsampled = upsampled.reshape(batch_size, seq_len * factor, d_model)
        elif self.method == 'linear':
            # Linear projection
            projected = self.linear(x)  # [batch, seq_len, d_model * factor]
            upsampled = projected.reshape(batch_size, seq_len * factor, d_model)
        elif self.method == 'attention':
        ...
        if shift_amount > 0:
            padding = torch.zeros(batch_size, shift_amount, d_model, device=x.device)
...
class CrossAttentionCondition(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        # Self-attention with residual
        attn_out, _ = self.attention(query, key, value, attn_mask=mask, need_weights=False)
...
class HourglassTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.point_cloud_encoder = PerceiverPointEncoder(d_model, input_dim, n_latents)

        self.use_static_routing = use_static_routing
        
        # Stage 1: Coordinate level
        self.stage1 = HourglassStage(stage_layers[0], d_model, n_heads, d_ff, dropout)
        self.stage1_conditioned = CrossAttentionCondition(d_model, n_heads, d_ff, dropout)
        ...
        self.stage2 = HourglassStage(stage_layers[1], d_model, n_heads, d_ff, dropout)
        self.stage2_conditioned = CrossAttentionCondition(d_model, n_heads, d_ff, dropout)

        # Shortening from vertex to face level (4x for quad vertices)
        self.shortening2 = ShorteningLayer(d_model, 4, shortening_method)
        
        # Stage 3: Face level
        self.stage3 = HourglassStage(stage_layers[2], d_model, n_heads, d_ff, dropout)
        self.stage3_conditioned = CrossAttentionCondition(d_model, n_heads, d_ff, dropout)
        ...
        self.stage4 = HourglassStage(stage_layers[3], d_model, n_heads, d_ff, dropout)
        self.stage4_conditioned = CrossAttentionCondition(d_model, n_heads, d_ff, dropout)

        # Upsampling layer from vertex coordinate level (2x 2D vertices)
        self.upsampling1 = UpsamplingLayer(d_model, 2, upsampling_method)
        
        self.stage5 = HourglassStage(stage_layers[4], d_model, n_heads, d_ff, dropout)
        self.stage5_conditioned = CrossAttentionCondition(d_model, n_heads, d_ff, dropout)
    ...
    def forward(
        ...
        latent_condition = self.point_cloud_encoder(point_cloud)
        # Stage 1: Coordinate level
        stage1_out = self.stage1(x, mask)
        stage1_conditioned_out = self.stage1_conditioned(stage1_out,latent_condition,latent_condition)
        ...
        stage2_conditioned_out = self.stage2_conditioned(stage2_out,latent_condition,latent_condition)
        ...
        stage3_conditioned_out = self.stage3_conditioned(stage3_out,latent_condition,latent_condition)

        # Upsampling 2: Face -> Vertex
        upsampled2 = self.upsampling2(stage3_conditioned_out, target_len=residual2.shape[1])
        ...
        stage4_conditioned_out = self.stage4_conditioned(stage4_out,latent_condition,latent_condition)

        # Upsampling 1: Vertex -> Coordinate
        upsampled1 = self.upsampling1(stage4_conditioned_out, target_len=seq_len)
        ...
        output_conditioned = self.stage5_conditioned(stage5_out,latent_condition,latent_condition)
